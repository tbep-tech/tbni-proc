...
CatchWithSegs <- 
  here(
    "data/TampaBay_NektonIndexTBEPSegAssign.rds"
  )

# Specify the species guild classification file
class <- 
  here(
    "data/TBIndex_spp_codes.rds"
  )
...
input_data <- 
  ...
  #now just keep one record of the total number of individuals of each species within a record
  distinct() %>%
...
NumTaxa <- 
  ...
  mutate(
    NumTaxa1 = n_distinct(NODCCODE)
  ) %>%
  #make no fish caught count as zero taxa
  mutate(
    NumTaxa = case_when(
      NODCCODE == "9998000000" ~ 0,
...
Diversity <- 
  ...
  filter(
    !NODCCODE == "9998000000" | !is.na(NumTaxa)
  ) %>%
  group_by(
    Reference
  ) %>%
...
NumGuilds <- 
  ...
  mutate(
    NumGuilds = n_distinct(Feeding_Guild)
  ) %>%
  select(
    -NODCCODE, 
    -Feeding_Guild
...
Dom <- 
  ...
  mutate(
    CumProp = cumsum(Total_N)/NumIndiv,
    Taxa    = row_number()
  ) %>%
  ungroup() %>%
...
TaxaCountPrep <- 
  ...
  mutate(
    TSTaxa = case_when(Feeding_Cat == "TS" ~ 1, TRUE ~ 0),
...
RefCond <- 
  ...
  summarize(
    NumTaxa_P5 = round(quantile(NumTaxa, probs = 0.05)),
...
TBNI.Scores <- 
  Metric.Scores %>%
  mutate(
    TBNI_Score = ((ScoreNumTaxa + ScoreShannon + ScoreTaxaSelect + ScoreTaxaBenthic + ScoreNumGuilds)/5)*10
  )

#save the scores to a file
saveRDS(
  TBNI.Scores, 
  paste0("data/TBNI_Scores_", min(TBNI.Scores$Year), "-", max(TBNI.Scores$Year), ".rds")
)
...
TBNI_Yrly_Scores <- 
  ...
  summarize(
    Yr_TBNI = round(mean(TBNI_Score),0),
...
TBNI_Seg_Scores <- 
  ...
  summarize(
    Segment_TBNI = round(mean(TBNI_Score),0)
  )
...
CommPerc <- 
  ...
  summarize(
    Score_P33 = quantile(TBNI_Score, probs = 0.33),
...
# Test what the new graph would look like:
...
TB_plot <- 
  ...
  geom_line(aes(x = Year, y = Yr_TBNI), color = "black", linewidth = 1.5) +
...
Seg_plot <- ggplot(TBNI_Seg_Scores) +
  ...
  geom_ribbon(aes(x = Year, ymin = RedColor, ymax = YellowColor), fill = "yellow", alpha = 3/10) +
  geom_ribbon(aes(x = Year, ymax = 58, ymin = YellowColor), fill = "green", alpha = 3/10) +
  geom_line(aes(x = Year, y = Segment_TBNI, linetype = bay_segment, color = bay_segment), linewidth = 1.25) +
