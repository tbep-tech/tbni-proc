#### Script Info ####
# Originally developed by: M Schrandt (April 10, 2020)
...
input <- 
  here(
    "data/TBNI_Scores_1998-2025.rds"
  )
...
AveBayScores <- 
  ...
  summarize(
    SegScore = round(mean(TBNI_Score))
  ) %>%
  mutate(
    MapCol = case_when(
      SegScore <= 32 ~ "#FF0000",
...
shape_TB <- 
  ...
  filter(
    bay_segment %in% c("OTB", "HB", "MTB", "LTB")
  ) %>%
  left_join(
    AveBayScores
  ) %>%
...
#### Make Map(s) ####
Yr_map <- 
  tm_shape(shape_TB_crop, bbox = bbox_new) +
  ...
  tm_scalebar(breaks = c(0,5,10,15), text.size = 1) +
